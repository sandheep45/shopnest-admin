...
export const CreateVariantSchema = z.object({
  description: z.string().min(16, { message: "Description must be at least 16 characters long" }),
  ...
  barcode: z.number().int().positive({ message: "Barcode must be a positive integer" }),
  taxPercent: z.number().min(0, { message: "Tax percent cannot be negative" }),
  onSelfQuantity: z.number().int().min(0, { message: "On self quantity cannot be negative" }),
...
export const CreateProductSchema = z.object({
  ...
  Category: z.object({
    connect: z.object({
      id: z.string().refine((val) => /^[a-f\d]{24}$/i.test(val), { message: "Invalid id" }),
    }),
  }),
  Variant: z.object({
  ...
  Category: z
    .object({
      connect: z.object({
        id: z
          .string()
          .refine((val) => /^[a-f\d]{24}$/i.test(val), { message: "Invalid category id" })
          .optional(),
      }),
    })
    .optional(),
